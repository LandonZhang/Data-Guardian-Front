...
async function sendMessage() {
  if (userInput.value.trim() && !isProcessing.value) {
    ...
    if (userMessage === '数据稽核开始') {
      ...
      try {
        ...
        if (response.data && response.data.answer) {
          ...
          // 使用正则表达式提取初始请求和最终请求
          const initialRequestMatch = answer.match(
            /用户初始请求是：([\s\S]*?)(?=\n最终确认请求是：|$)/,
          )
    ...
    // 构建包含项目信息的查询
    const projectPrefix = props.selectedProject ? `在${props.selectedProject}项目的背景下，` : ''
          ...
          <template v-else-if="message.isAudit">
            <div class="audit-container">
              <div class="audit-content">
                ...
                <div class="progress-container">
                  <div class="progress-bar">
                    <div
                      class="progress-fill"
                      :style="{ width: message.auditProgress + '%' }"
                    ></div>
                  </div>
              ...
              <div class="audit-result" v-if="message.auditProgress >= 100">
                感谢您的等待，本次稽核已完成，<a href="#" class="audit-link">点击查看稽核报告</a
                >。如需再次开启稽核，请点击右下角新建对话。
              </div>
            </div>
          </template>
